...
// Сбросить пароль
app.post("/newpassword", (request, response)=> {newpassContrl.newpassword(request, response)})

// Зарегистрироваться
app.post("/signup", (request, response)=> {signupContrl.signup(request, response)})

// Вход в аккаунт
app.post("/login", (request, response)=> {loginContrl.login(request, response)})

app.post("/load",  passport.authenticate('jwt', { session: false }), (request, response)=> {loadContrl.loadData(request, response)})
...
app.post("/home",  (request, response)=> {homeContrl.home(request, response)})
...
cron.schedule(TIME_STOP_CRON_TASKS, () => 
{
...
cron.schedule(TIME_START_CRON_TASKS, () =>
{
